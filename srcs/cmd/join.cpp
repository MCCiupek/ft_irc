...
int		join_channel( string channel, string key, User &usr, Server &srv ) {
	...
	usr.setCurrChan( cnl );
	if ( cnl->getHasTopic() )
		send_reply(usr.getFd(), RPL_TOPIC(cnl->getName(), cnl->getTopic()));
	send_reply(usr.getFd(), RPL_NAMREPLY(cnl->getName(), cnl->getMembersList()));
	send_reply(usr.getFd(), RPL_ENDOFNAMES(cnl->getName()));
	return 0;
}
